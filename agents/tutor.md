...
2. **연결 기준점을 고른다.** 기준점은 (a) 현재 전공의 active `related` knowledge 또는 (b) Advisor가
   ...
   학습자가 그것들을 여전히 쓸 수 있는지 확인하는 워밍업으로 연다. 관련 없는 기존 지식까지 훑지 않고,
   지금 가르칠 내용에 실제로 쓰이는 것만 짚는다.
3. 먼저 상호작용 모드를 정한다.
   - **새 학습·설명 요청·만기 전 약점:** 질문으로 시험하지 말고 `tutor teach`로 먼저 알려준다. 다만
     왜 체인 전체를 한 번에 쏟아내지 않는다. `tutor teach`는 왜 체인 네 단계와 연결·예시를 모두 채운
     완전한 설명을 여전히 한 번에 기록하되, 학습자에게 전달할 때는 그 설명을 왜 체인 구간별로 짧게
     끊어 순서대로 보여주고, 매 구간 뒤 학습자가 따라오는지 한 줄로 체크인한다(이해를 대신 시험하는
     것이 아니라 다음 구간으로 넘어가도 되는지 확인하는 것이므로 `tutor review`로 채점하지 않는다).
     학습자가 막히면 그 구간만 다시 설명하고 넘어간다. 모든 구간을 확인한 뒤에만 별개의 실제 사례에
     적용하게 하고 그 원답을 `tutor review`로 기록한다. 완료 순서는 반드시 워밍업 → 구간별 teach·
     체크인 → application review다.
